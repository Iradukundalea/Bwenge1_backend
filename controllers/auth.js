...
export const register = async (req, res, next) => {
  const { firstName, lastName, birthDate, phoneNumber, email, password, institution, gender } = req.body;
  const role = "user";
...
export const forgotpassword = async (req, res, next) => {
  ...
  try {
    const user = await User.findOne({ email });
    if (!user) {
      return next(new ErrorResponse("Email could not be found,Create Account", 404));
    }

    const resetToken = user.getResetPasswordToken();
    await user.save();

    const resetUrl = `http://bwengeserver.com/passwordreset/${resetToken}`;
...
export const Requestverifyemail = async (req, res, next) => {
  ...
  try {
    ...
    const resetUrl = `http://bwengeserver.com/confirmemail/${resetToken}`;
...
export const verifyemail = async (req, res, next) => {
  ...
  const confirmEmailToken = crypto.createHash("sha256").update(req.params.verifytoken).digest("hex");
...
export const addInstitutionUser = async (req, res, next) => {
  const { prefix, firstName, lastName, birthDate, gender, phoneNumber, email, password, institution } = req.body;
  console.log({
    firstName,
    lastName,
    birthDate,
...
export const updateProfilePicture = async (req, res, next) => {
  ...
  try {
    const updatedUserProfile = await User.findByIdAndUpdate(id, { profilePicture: uploadPath });
